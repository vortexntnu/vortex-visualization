...
def generate_launch_description():
    urdf_path = PathJoinSubstitution(['urdf', 'orca', 'AUV_MainAssembly.urdf'])
    ...
    # base_link (NED) to orca_model
    tf_base_link_to_orca_model = Node(
        package='tf2_ros',
        ...
        arguments=['--x'             , '0',
                   ...
                   '--yaw'           , '0',
                   '--frame-id'      , 'base_link',
                   '--child-frame-id', 'orca_model']
    )

    return LaunchDescription([
        urdf_launch,
        tf_base_link_to_orca_model
    ])
