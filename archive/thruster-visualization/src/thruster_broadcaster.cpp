...
ThrusterVisualization::ThrusterVisualization()
    : Node("thruster_visualization_node") {
  thruster_marker_publisher_ =
      this->create_publisher<visualization_msgs::msg::MarkerArray>(
          "visualization_marker", 1);
  thruster_forces_subscriber_ =
      this->create_subscription<std_msgs::msg::Float32MultiArray>(
          "thrust/thruster_forces", 10,
          std::bind(&ThrusterVisualization::thruster_forces_callback, this,
                    std::placeholders::_1));
  ...
  thruster_positions_ = std::vector<std::vector<double>>(
      num_thrusters_, std::vector<double>(3, 0.0));
  thruster_orientations_ = std::vector<double>(num_thrusters_, 0.0);

  for (int i = 0; i < num_thrusters_; ++i) {
    this->declare_parameter<std::vector<double>>(
        "thruster" + std::to_string(i) + "_position");
    this->declare_parameter<double>("thruster" + std::to_string(i) +
                                    "_orientation");

    this->get_parameter("thruster" + std::to_string(i) + "_position",
                        thruster_positions_[i]);
    this->get_parameter("thruster" + std::to_string(i) + "_orientation",
                        thruster_orientations_[i]);
  }
...
void ThrusterVisualization::publish_markers() {
  ...
  for (size_t i = 0; i < thruster_data_.size(); ++i) {
    ...
    double torque_z = thruster_positions_[i][0] * force_y -
                      thruster_positions_[i][1] * force_x;
  ...
  visualization_msgs::msg::Marker total_force_marker =
      create_total_force_marker(total_force_magnitude_,
                                total_force_orientation_);
  marker_array.markers.push_back(total_force_marker);

  // Visualize the total torque
  double normalized_torque =
      std::min(std::abs(total_torque_z) / max_torque, 1.0);
  ...
  visualization_msgs::msg::Marker arc =
      create_arc_marker("base_link", thruster_data_.size() + 1, center, radius,
  ...
  visualization_msgs::msg::Marker torque_direction_arrow =
      create_torque_direction_marker("base_link", thruster_data_.size() + 2,
                                     center, radius, final_angle,
                                     total_torque_z);
...
void ThrusterVisualization::thruster_forces_callback(
...
ThrusterVisualization::create_total_force_marker(
    double total_force_magnitude, double total_force_orientation) {
  visualization_msgs::msg::Marker total_force_marker;
...
visualization_msgs::msg::Marker ThrusterVisualization::create_arc_marker(
    ...
    const geometry_msgs::msg::Point &center, double radius, double start_angle,
    double end_angle, int num_segments) {
  visualization_msgs::msg::Marker arc;
...
visualization_msgs::msg::Marker
ThrusterVisualization::create_torque_direction_marker(
    const std::string &frame_id, int id,
    const geometry_msgs::msg::Point &center, double radius, double final_angle,
