...
async function prepublish(extensionPath, pkg) {
    ...
    (0, log_1.info)(`Executing prepublish script 'yarn run foxglove:prepublish'...`);
...
async function install(files, extensionPath, pkg) {
    ...
    const destDir = (0, path_1.join)((0, os_1.homedir)(), ".foxglove-studio", "extensions", dirName);
    await rmdir(destDir);
...
async function rmdir(dirname) {
    return await new Promise((c, e) => (0, rimraf_1.default)(dirname, (err) => (err != undefined ? e(err) : c())));
}
...
function addFileToZip(zip, baseDir, filename) {
    ...
    zip.file(filename, (0, fs_1.createReadStream)(fullPath), { createFolders: true, date: MOD_DATE });
}
